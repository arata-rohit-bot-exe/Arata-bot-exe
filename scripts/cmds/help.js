const fs = require("fs-extra"); const axios = require("axios"); const path = require("path"); const { getPrefix } = global.utils; const { commands, aliases } = global.GoatBot;

module.exports = { config: { name: "help", version: "2.0", author: "rifat", countDown: 5, role: 0, shortDescription: { en: "Show help menu or command details" }, longDescription: { en: "Display all available commands or detailed information about a specific command" }, category: "info", guide: { en: "{pn} [command name]" }, priority: 1 },

onStart: async function ({ message, args, event, threadsData, role }) { const { threadID } = event; const prefix = getPrefix(threadID); const threadData = await threadsData.get(threadID);
...
function roleTextToString(role) { switch (role) { case 0: return "0 - All Users"; case 1: return "1 - Group Admins"; case 2: return "2 - Bot Admins"; default: return "Unknown Role"; } }
